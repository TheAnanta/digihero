...
class GameService extends ChangeNotifier {
  ...
  // Level Progress
  Map<int, Map<String, dynamic>> _levelProgress = {};
  ...
  void completeLevel(int level, int score, int stars, int timeSeconds) {
    if (_levelProgress[level] != null) {
      ...
      if (_levelProgress[level]!['bestTime'] == 0 || timeSeconds < _levelProgress[level]!['bestTime']) {
        _levelProgress[level]!['bestTime'] = timeSeconds;
      }
      ...
      if (stars > _levelProgress[level]!['stars']) {
        ...
        int newStars = stars - _levelProgress[level]!['stars'];
      ...
      // Unlock next level
      if (level < AppConstants.totalLevels && _levelProgress[level + 1] != null) {
